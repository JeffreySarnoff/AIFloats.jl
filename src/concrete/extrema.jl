firstNonzeroPrenormalMagnitude(T::Type{<:AbstractAIFloat}) = 1 / nmags_prenormal(T)
lastPrenormalMagnitude(T::Type{<:AbstractAIFloat}) = (nmags_prenormal(T) - 1) / nmags_prenormal(T)
...
mag_normal_max(T::Type{<:AbstractUnsigned}) = exp_value_max(T) * (1 + ((nmags_prenormal(T) - 1) - 1 - is_extended(T)) / nmags_prenormal(T))
mag_normal_max(T::Type{<:AbstractSigned}) = exp_value_max(T) * (1 + ((nmags_prenormal(T) - 1) - is_extended(T)) / nmags_prenormal(T))
