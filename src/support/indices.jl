...
code_one(T::Type{<:AbstractUnsigned}) = ((n_values(T) % UInt16) >> 0x0001)
code_one(T::Type{<:AbstractSigned}) = ((n_values(T) % UInt16) >> 0x0002)
code_negone(T::Type{<:AbstractSigned}) = (((n_values(T) % UInt16) >> 0x0002) + n_values(T)>>1)
code_negone(T::Type{<:AbstractUnsigned}) = nothing # throw(DomainError(T, "index_negone: T must be an AbstractSigned type, not $T"))

code_nan(T::Type{<:AbstractUnsigned}) = ((n_values(T) % UInt16) - 0x0001)
code_nan(T::Type{<:AbstractSigned}) = ((n_values(T) % UInt16) >> 0x0001)
 
code_posinf(T::Type{<:AkoUnsignedExtended}) = ((n_values(T) - 1) % UInt16)
code_posinf(T::Type{<:AkoSignedExtended}) = ((n_values(T) % UInt16) >> 0x0001)
code_posinf(T::Type{<:AkoUnsignedFinite}) = nothing
code_posinf(T::Type{<:AkoSignedFinite}) = nothing

code_neginf(T::Type{<:AkoSignedExtended}) = (n_values(T) % UInt16)
