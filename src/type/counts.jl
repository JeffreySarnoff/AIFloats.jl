...
nNegInfs(@nospecialize(T::Type{<:AbstractSignedFloat}))   = (nInfs(T) + 1) >> 1
nNegInfs(@nospecialize(T::Type{<:AbstractUnsignedFloat})) = 0
...
nSignBits(@nospecialize(T::Type{<:AbstractSignedFloat})) = 1
nSignBits(@nospecialize(T::Type{<:AbstractUnsignedFloat})) = 0

nExpBits(@nospecialize(T::Type{<:AbstractSignedFloat})) = nBits(T) - nSigBits(T)
nExpBits(@nospecialize(T::Type{<:AbstractUnsignedFloat})) = nBits(T) - nSigBits(T) + 1

nMagnitudes(T::Type{<:AbstractUnsignedFloat}) = 2^(nBits(T)) - 1
nMagnitudes(T::Type{<:AbstractSignedFloat})   = 2^(nBits(T)  - 1)
...
nPositiveValues(::Type{T}) where {Bits, SigBits, T<:AbstractSignedFloat{Bits, SigBits}} = nNonnegValues(T) - 1
nNegativeValues(::Type{T}) where {Bits, SigBits, T<:AbstractSignedFloat{Bits, SigBits}} = nNumericValues(T) - nNonnegValues(T)
...
nPrenormalValues(::Type{T}) where {Bits, SigBits, T<:AbstractSignedFloat{Bits, SigBits}} = 2 * nPrenormalMagnitudes(T) - 1
nPrenormalValues(::Type{T}) where {Bits, SigBits, T<:AbstractUnsignedFloat{Bits, SigBits}} = nPrenormalMagnitudes(T)
