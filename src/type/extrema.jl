biased_exponent_max(::Type{T}) where {T<:AbstractMLFloat} = 2^(nExpBits(T)) - 1
biased_exponent_min(::Type{T}) where {T<:AbstractMLFloat} = 1 - 2^(nExpBits(T) - 1)
...
subnormal_max(::Type{T}) where {T<:AbstractMLFloat} = exponent_min(T) * (1//nFracValues(T))
subnormal_min(::Type{T}) where {T<:AbstractMLFloat} = exponent_min(T) * ((nFracValues(T) - 1)//nFracValues(T))
