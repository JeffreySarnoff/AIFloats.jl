...
function round_tozero(xs::T, x::F) where {T<:AbstractUnsigned, F<:AbstractFloat}
    round_down(xs, x)
end

function round_fromzero(xs::T, x::F) where {T<:AbstractUnsigned, F<:AbstractFloat}
    round_up(xs, x)
end
...
function round_nearesteven(xs::T, x::F) where {T<:AbstractUnsigned, F<:AbstractFloat}
    ...
    elseif dval0 > dval1
        ...
        even_bits0 = trailing_zeros((idx0-1) % typeforcode(n_bits(T)))
        even_bits1 = trailing_zeros((idx1-1) % typeforcode(n_bits(T)))
        if even_bits0 > even_bits1
...
function round_nearestaway(xs::T, x::F) where {T<:AbstractUnsigned, F<:AbstractFloat}
    round_nearestfromzero(xs, x)
end

# rounding for T<:AbstractSigned
