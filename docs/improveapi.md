...
AIFloats should make one clean, breaking API change while it is at version
`0.1.0`:
...
3. Capture Chairmarks baselines for constructor, conversion, default lookup,
   ...
   allocations, bytes, and throughput.
...
### Phase 1 — make the format and datum model canonical
...
5. Refactor `BinaryValue` so its public numeric constructors all call the value
   conversion seam. Remove the `Unsigned` code-point constructors.
6. Add checked `fromcode` methods for a format type and datum type. Rename
   `rawvalue` to `_rawvalue`, make its input contract exact, and remove it from
   `public`.
7. Migrate internal raw-code call sites only after classifying them in Phase 0.
   Migrate tests that intentionally specify codes to `fromcode`; leave actual
   numeric construction as ordinary calls.
8. Update named-format use sites. Code such as `T = Binary8p4se` that needs an
   element type becomes `F = Binary8p4se; T = BinaryValue(F)`. Code that only
   constructs values can continue to call `Binary8p4se(x)`.
9. Update the precompile workload so it explicitly names `F` and `T`, and
   precompiles `F(x)`, `T(x)`, `fromcode(F,c)`, and `Convert(F,rho,x)`.
10. Run method-ambiguity and inference checks immediately. Overloading calls on
...
6. Preserve the `rho === RTE_SN` specialization guard where it measurably helps,
   but read the scoped value only in convenience wrappers. Explicit projection
   methods must remain identical to their current implementation.
...
3. Update examples so every code literal is visibly introduced by `fromcode`;
   otherwise integers and unsigned integers are visibly numeric values.
4. Update benchmark helpers to use scoped projection and the new table
   snapshots. Do not reach into scoped values, caches, or container fields.
5. Update conformance reports and `formatname`/`codetable` to accept format
   types, datum types, and datums through their documented query seam, without
   reintroducing format instances.
6. Update exports/public declarations. An exported name must have a docstring;
   a private raw path and internal cache implementation must not be `public`.
7. Run focused tests and benchmarks listed below. Do not run the full test suite
...
code points for each deterministic projection.
...
Run individual files or small related groups, not `Pkg.test()`:
...
3. default and explicit `Add`, `Exp`, and `Convert`;
4. a `with_projection` scope setup and repeated operations inside one scope;
...
- Scoped default lookup: at most once per public call and never per array
  element. Repeated work inside one `with_projection` scope should not allocate
  per operation.
...
requires semantic inventory before deletion and a repository-wide migration to
`fromcode`, followed by an exhaustive search for residual raw-code constructor
calls.
...
Explicit-projection functions remain pure with respect to task context.
...
No compatibility phase remains. The only dual names retained are permanent
P3109/Julia query bindings that share one function object; they are part of the
target API rather than migration adapters.
...
projection paths do not consult scoped state, array methods resolve context
once, dynamic operation validation crosses a function barrier once, and raw
constructors remain available only inside proved loops. Cache statistics and
serialization copying are kept off arithmetic hot paths.
