...
explicit complementary guards. Finite operands are `fin(R)` constructor
terms, so the finite rows of every ω operation are pattern-disjoint from the
exceptional rows by sort alone, and the kind of `XReal` contains no built-in
operator. Unknown symbolic tests are never treated as
