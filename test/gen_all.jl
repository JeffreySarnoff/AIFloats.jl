...
for idx in 1:length(ufvals)
    for idx2 in 1:length(ufvals[idx])
        str = string(ufvals[idx][idx2])
...
for idx in 1:length(uevals)
    for idx2 in 1:length(uevals[idx])
        str = string(uevals[idx][idx2])
        if str === "nan" || endswith(str, "inf")
            if str == "nan"
                uevals[idx][idx2] = NaN
            elseif str == "+inf"  || str == "inf"  
...
for idx in 1:length(sfvals)
    for idx2 in 1:length(sfvals[idx])
        str = string(sfvals[idx][idx2])
        if str === "nan" || endswith(str, "inf")
            if str == "nan"
                sfvals[idx][idx2] = NaN
            elseif str == "+inf"  || str == "inf"  
...
for idx in 1:length(sevals)
    for idx2 in 1:length(sevals[idx])
        str = string(sevals[idx][idx2])
        if str === "nan" || endswith(str, "inf")
            if str == "nan"
                sevals[idx][idx2] = NaN
            elseif str == "+inf"  || str == "inf"  
...
using Tables, CSV, Printf, PrettyTables, Latexify, JLD2
